...
func normalizeDRWAAuthorizedCallerAddress(value string) ([]byte, error) {
	...
	// A 32-character all-hex string is NOT a valid raw address — it is rejected.
	// Valid formats: 64 hex chars (32-byte address) or 32 raw bytes.
	// Disambiguate: if the string is 32 chars but all are valid hex chars,
	// it decodes to only 16 bytes — not a valid 32-byte address — so it falls
	// through to the hex decode path below which requires exactly 32 decoded bytes.
	// Only treat as raw bytes if it contains non-hex characters (indicating
	// actual binary content, i.e. a genuine 32-byte raw binary address).
	if len(address) == 32 {
		_, hexErr := hex.DecodeString(address)
		if hexErr != nil {
			// Contains non-hex chars → genuine 32-byte raw binary address
			raw := []byte(address)
		...
		// All hex chars → fall through to hex decode path below (64-char hex expected for 32-byte address)
	}
