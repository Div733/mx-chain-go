...
// PI-01: These thresholds are hardcoded for initial deployment. Future releases
// should make them configurable per-token-policy via governance, allowing
// operators to adjust failure tolerance without protocol upgrades.
const (
	...
	// This is an absolute count upper bound, not a rate. For large token deployments
	// (1000+ holders), consider converting to a basis-points rate like the other thresholds.
	// Current value is conservative for initial rollout phases.
...
func validateDRWARolloutManifest(manifest *drwaRolloutManifest) error {
	...
	// Only reject values exceeding the upper bound cap.
