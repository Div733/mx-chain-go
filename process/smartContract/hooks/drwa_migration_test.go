...
func TestPersistDRWAMigrationAuthorizedCallers(t *testing.T) {
	...
	adapter := newMockDRWASyncStateAdapter()
	// Use non-hex characters so these are treated as raw 32-byte addresses
	// (hex-only 32-char strings are ambiguous — see M-1 documentation).
	policyRaw := strings.Repeat("p", 32)
	assetRaw := strings.Repeat("z", 32)
	identityRaw := strings.Repeat("i", 32)
	attestationRaw := strings.Repeat("t", 32)
	...
	manifest := &drwaMigrationManifest{
		...
		AuthorizedCallers: map[string]string{
			...
			drwaSyncCallerIdentityRegistry: identityRaw,
			drwaSyncCallerAttestation:  attestationRaw,
	...
	if got := string(adapter.authorizedCallers[drwaSyncCallerIdentityRegistry]); got != identityRaw {
		t.Fatalf("unexpected identity caller: %s", got)
	}
	if got := string(adapter.authorizedCallers[drwaSyncCallerAttestation]); got != attestationRaw {
		t.Fatalf("unexpected attestation caller: %s", got)
	}
